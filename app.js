/**
 * Academic Vocabulary App — Units 1–11 + Grammar
...
(function () {
    ...
    const UNIT_NAMES = {
        ...
        11: 'Unit 11: Technology & Innovation'
    };
    ...
    function openGrammar() {
        ...
        document.title = 'Grammar — Tenses & Units 1–11';
    ...
    function openProgress() {
        ...
        $('progressView')?.classList.add('is-active');
    ...
    function refreshProgressPanel() {
        ...
        const book = user.profile?.selectedBook?.title || 'Not selected';
        $('progressCoins') && ($('progressCoins').textContent = String(coins));
        $('progressBook') && ($('progressBook').textContent = book);
        $('progressUsername') &&
    ...
    function initGrammarApp() {
        ...
        const unitSelect = $('grammarUnitFilter');
        if (unitSelect && unitSelect.options.length <= 1) {
            grammarByUnit.forEach((unit) => {
                const opt = document.createElement('option');
                opt.value = unit.u;
                opt.textContent = UNIT_NAMES[unit.u] || 'Unit ' + unit.u;
    ...
    function bindHome() {
        ...
        if (vocabCard) {
            vocabCard.addEventListener('click', () => openVocab());
        ...
        const gameCard = $('gameModuleCard');
        if (gameCard && window.GameModule) {
            gameCard.addEventListener('click', window.GameModule.openGame);
            gameCard.addEventListener('keydown', (e) => {
                if (e.key === 'Enter' || e.key === ' ') {
                    e.preventDefault();
                    window.GameModule.openGame();
                }
            });
        }

        const progressCard = $('progressModuleCard');
        if (progressCard) {
            progressCard.addEventListener('click', openProgress);
        ...
        $('btnGameBack')?.addEventListener('click', goHome);
        ...
        $('btnChangeBook')?.addEventListener('click', () => openChangeBookModal().catch(() => {}));
        $('btnVocabChangeBook')?.addEventListener('click', () => openChangeBookModal().catch(() => {}));
        $('btnProgressBack')?.addEventListener('click', goHome);
        $('btnProgressChangeBook')?.addEventListener('click', () =>
            openChangeBookModal().then(refreshProgressPanel).catch(() => {})
        );
        ...
        document.addEventListener('keydown', (e) => {
            ...
            if ($('gameView')?.classList.contains('is-active') && e.key === 'Escape') {
                goHome();
            }
            if ($('progressView')?.classList.contains('is-active') && e.key === 'Escape') {
                goHome();
            }
        });
    }
    ...
    function openChangeBookModal() {
        return new Promise((resolve, reject) => {
            const modal = $('changeBookModal');
            const list = $('changeBookModalList');
            if (!modal || !list) {
                reject(new Error('Book modal not found'));
                return;
            }

            window.AppAuth.api('/books').then(({ books }) => {
                if (!books?.length) {
                    reject(new Error('No vocabulary books available.'));
                    return;
                }

                const currentBookId = window.AppAuth.getUser?.()?.profile?.selectedBook?.id;

                list.innerHTML = books
                    .map(
                        (b) => `
                    <label class="book-option">
                        <input type="radio" name="changeVocabBook" value="${b.id}" data-slug="${escapeAttr(b.slug)}">
                ...
                let selected = list.querySelector(`input[value="${currentBookId}"]`);
                if (!selected && inputs[0]) selected = inputs[0];
                ...
                const onConfirm = async () => {
                    const selected = list.querySelector('input[name="changeVocabBook"]:checked');
                    const modalErr = $('changeBookModalError');
                    if (!selected) {
                    ...
                    $('changeBookModalConfirm').disabled = true;
                    try {
                        const data = await window.AppAuth.api('/books/select', {
                            method: 'POST',
                            body: JSON.stringify({
                        ...
                        if (!data?.user?.profile?.selectedBook) {
                            throw new Error('Could not save book choice.');
                        }
                        window.AppAuth.updateCurrentUser?.(data.user);
                        setVocabBookLabel(data.user.profile.selectedBook);
                        refreshProgressPanel();
                        closeModal();
                        resolve(data.user.profile.selectedBook);
                    } catch (err) {
                        modalErr.textContent = err.message;
                        modalErr.classList.add('is-visible');
                    } finally {
                        $('changeBookModalConfirm').disabled = false;
                    }
                };
                ...
                $('changeBookModalConfirm').addEventListener('click', onConfirm, { once: true });
                $('changeBookModalCancel')?.addEventListener('click', onCancel, { once: true });
            }).catch(reject);
        });
    }

    window.App = { goHome, openProgress, refreshProgressPanel };
